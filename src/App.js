        ...
        <div className="score-container">
          ...
          <div className="second-container">
            <img
              ...
              className="time-logo"
            />
            <p className="second">{seconds} sec</p>
          </div>
        </div>
      </div>
    </nav>
...
const ThumbnailItem = props => {
  const {thumbnailDetails, checkTheScore} = props
  ...
  return (
    <li className="thumbnail-item">
      <button type="button" onClick={onClickButton} className="thumbnail-btn">
  ...
  return (
    <div className="game-over-bg-container">
      <img
        ...
        className="trophy"
      />
      <p className="end-game-title">YOUR SCORE</p>
      <p className="end-game-score">{score}</p>
...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.decreaseSecond, 1000)
  }
  ...
  getActiveTabImage = () => {
    ...
    const filteredImage = imagesList.filter(
      eachOb => eachOb.category === activeTabId,
    )
    return filteredImage
  }

  checkTheScore = clickedId => {
    const {randomIndex, seconds} = this.state
    const {id} = imagesList[randomIndex]
    const randomNo = Math.ceil(Math.random() * imagesList.length - 1)

    if (id === clickedId) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomIndex: randomNo,
      }))
    }
    if (seconds === 0) {
      clearInterval(this.timerId)
  ...
  render() {
    const {score, seconds, activeTabId, randomIndex} = this.state
    const {imageUrl} = imagesList[randomIndex]
    const filteredImage = this.getActiveTabImage()
    return (
      <>
        <div className="app-container">
          ...
          <div className="app-game">
            <div className="inner-container">
              {seconds === 0 ? (
                <GameOverCard score={score} replayAgain={this.replayAgain} />
              ) : (
                <div>
                  <img src={imageUrl} alt="match" className="game-image" />
                  ...
                  <ul className="thumbnail-list-container">
                    {filteredImage.map(eachOb => (
                      <ThumbnailItem
                        thumbnailDetails={eachOb}
                        key={eachOb.id}
                        checkTheScore={this.checkTheScore}
                      />
                    ))}
                  </ul>
                </div>
